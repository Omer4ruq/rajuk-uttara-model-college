...
export default function AcademicDocumentsDownload() {
  
  const [documents, setDocuments] = useState([]);
  const documentType = ["Assignment", "Hand Book", "Home Work", "Class Notes", "Other Download"];
  ...
  // states to fill-up by fetching data
   ...
   function handleClick(){
    let link;    
    if (!(selectedClass && selectedYearType && selectedDocumentType)){
      alert("Please select all form fields");
      return;
    }
    
    if(selectedYearType == "session"){
    link = `/downloads/?class_id=${selectedClass}&academic_session_year__start_year=${selectedTimeline}&download_type=${selectedDocumentType}`;
    } else {
      link = `/downloads/?class_id=${selectedClass}&academic_year__name=${selectedTimeline}&download_type=${selectedDocumentType}`;
    }
  ...
  return (
   <div>
    ...
    <form className="flex flex-wrap justify-center items-center text-primary">
      ...
      {/* Document Type */}
      <div className="w-full md:w-1/3 px-2 mb-4">
        <label className="block text-primary text-2xl mb-2">Document Type</label>
        <select value={selectedDocumentType} onChange={(e) => setSelectedDocumentType(e.target.value)} className="select select-bordered w-full text-xl border border-primary focus:border-primary focus:outline-none">
          <option disabled selected>
            Document Type
          </option>
          {
    ...
    {documents?.length > 0 && <DownloadTable data={documents} />}
