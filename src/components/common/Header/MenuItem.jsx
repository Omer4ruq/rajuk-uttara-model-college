...
const LargeMenu = ({ menuItem }) => {
  ...
  return (
    <li className="relative group mx-1 my-2">
      <Wrapper
        to={menuItem.link}
        className={`flex items-center justify-between gap-2 rounded-md text-menuColor px-3 py-3 hover:bg-menuColor hover:text-white w-full h-full transition-all duration-200 ${
          menuItem.link ? "" : "cursor-pointer"
        }`}
      >
        <span className="whitespace-nowrap">{menuItem.title}</span>
        {!menuItem.link && (
          <GoTriangleDown className="group-hover:rotate-180 duration-200 flex-shrink-0" />
        )}
      </Wrapper>

      {menuItem.children && (
        <ul className="hidden group-hover:block absolute left-0 top-full mt-1 bg-white z-50 w-48 xl:w-60 divide-y divide-gray-100 text-start shadow-xl border border-gray-200 rounded-md transition-all duration-200 delay-75 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0">
          {menuItem.children.map((dropdown, index) => {
            ...
            return (
              <li key={index} className="relative group/dropdown">
                ...
                {dropdown.children && (
                  <ul className="hidden group-hover/dropdown:block bg-white divide-y divide-gray-100 w-48 xl:w-60 absolute left-full top-0 shadow-xl border border-gray-200 rounded-md transition-all duration-200 delay-75 opacity-0 group-hover/dropdown:opacity-100 transform translate-x-2 group-hover/dropdown:translate-x-0 z-50">
                    {dropdown.children.map((level2, idx) => (
                      <li key={idx}>
                        <Link
                          to={level2.link}
                          className={`block w-full px-4 py-3 bg-white text-menuColor hover:bg-menuColor hover:text-white transition-all duration-200 ${
                            idx === 0 ? "rounded-t-md" : ""
